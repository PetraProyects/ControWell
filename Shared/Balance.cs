...
namespace ControWell.Shared
{
    public class Balance
    {
        ...
        public double Ctsh()
        {
            double? tempFluido = TemTanqueCorregido();
            double? tempAmbiente = TemAmbiente;
            double? tempBase = Tanque.TBase;
            ...
            double ctsh = Math.Round(1 + (2 * coefMaterial * (((Math.Round(((7 * (double)tempFluido) + (double)tempAmbiente) / 8, 0))) - (double)tempBase)) + (Math.Pow(coefMaterial, 2) * Math.Pow(((Math.Round(((7 * (double)tempFluido) + (double)tempAmbiente) / 8, 0)) - (double)tempAmbiente), 2)), 5);
            return ctsh;
        }
        public double GOV()
        {

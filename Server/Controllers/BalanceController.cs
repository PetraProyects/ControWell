...
namespace ControWell.Server.Controllers
{
    ...
    public class BalanceController : ControllerBase
    {
        ...
        [HttpPost("Validar")]
        public async Task<ActionResult<Balance>> SimularObjetoBalance([FromBody] Balance mibalance)
        {
            ...
            var AforoCorrespondiente = AforosDelTanque.Where(x => x.Nivel == Math.Round((double)mibalance.NivelCorregido(), 1)).FirstOrDefault();
            ...
            var AforoCorrespondienteAgua = AforosDelTanque.Where(x => x.Nivel == Math.Round((double)mibalance.InterfaseCorregida(), 1)).FirstOrDefault();
            ...
            if (balancesDelTan.Count > 0)
            {
                ...
                mibalance.DeltaNsv = Math.Round(mibalance.NSV() - Ultimo.NSV(), 2);
        ...
        [HttpPost("Confirmar")]        
        public async Task<ActionResult<Balance>> CrearObjetoBalance([FromBody] Balance mibalance)
        {
            Tanque? mitanque = new Tanque();
            Termometro? mitermometro = new Termometro();
            Cinta? micinta = new Cinta();
